...
void eval(char *cmdline) 
{
    ...
    int stdin_redir[26]; 
    int stdout_redir[26];
    int amfersan = parseline(cmdline,argv); 
    int evaluate = builtin_cmd(argv);
    ...
    if(evaluate == 0){
        ...
        int state = amfersan ? 2 : 1;
        parseargs(argv,cmds,stdin_redir,stdout_redir);
        pid_t  pid = fork();
        if (pid ==0){
            sigprocmask(SIG_SETMASK, &prev_mask, NULL);
            execv(argv[0],&argv[cmds[0]]);
            printf("%s: Command not found\n",argv[0]);
            exit(1);
        }
        else{
            setpgid(pid, pid);
            addjob(jobs,pid,pid,state,cmdline);
            sigprocmask(SIG_SETMASK, &prev_mask, NULL);
            if(amfersan){
...
void do_bgfg(char **argv) 
{
    if(argv[1]== NULL ){
    ...
    }else if (argv[1][1] != '%' &&  !(argv[1][1] >='0' && argv[1][1]<='9')){
        ...
        if(strcmp(argv[0],"fg") == 0){
            if(argv[1][0]=='%'){
                job = atoi(&argv[1][1]);
                struct job_t* myJob = getjobjid(jobs,job);
                if(myJob != NULL){
                    myJob->state = FG;
                ...
                struct job_t* myJob = getjobpid(jobs,job);
                if(myJob!= NULL){
                    myJob->state = FG;
                    //printf("[%d] (%d) %s ",myJob->jid,myJob->pid,myJob->cmdline);
                    kill(-myJob->pid,SIGCONT);
...
{
    if (verbose){
        printf("sigstop_handler: entering\n");
    }
	pid_t pid = fgpid(jobs);
    if(pid >0 ){
        kill(-pid,SIGSTOP);
