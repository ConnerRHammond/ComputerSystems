...
void eval(char *cmdline) 
{
    ...
    int num_commands = parseargs(argv,cmds,stdin_redir,stdout_redir);
        if(num_commands == 1) {
            ...
            if(pid == 0){
                if(stdin_redir[0] != -1){
                    FILE * myFile1 = fopen(argv[stdin_redir[0]], "r");
                    int descriptor = fileno(myFile1);
                    dup2(descriptor,0);
                    close(descriptor);
                }
                if(stdout_redir[0] != -1){
                    FILE * myFile2 = fopen(argv[stdout_redir[0]], "w");
                    int descriptor = fileno(myFile2);
                    dup2(descriptor,1);
                    close(descriptor);
                }
            ...
            for(int i  = 0; i< num_commands; i++ ){
                ...
                if(pid == 0){
                    if(i == 0 && stdin_redir[i] != -1){
                        FILE * myFile1 = fopen(argv[stdin_redir[i]], "r");
                        int descriptor = fileno(myFile1);
                        dup2(descriptor,0);
                        close(descriptor);
                    }
                    ...
                    if(i == num_commands-1 && stdout_redir[i] != -1){
                        FILE * myFile2 = fopen(argv[stdout_redir[i]], "w");
                        int descriptor = fileno(myFile2);
                        dup2(descriptor,1);
                        close(descriptor);
                    }
